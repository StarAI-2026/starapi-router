...
### Changed
- **全面去 CLIProxyAPI 品牌化（配置/Docker/模块名）**：`go.mod` 从 `github.com/router-for-me/CLIProxyAPI/v7` 迁至 `github.com/StarAI-2026/starapi-router-private/v7`，全仓 import 与示例/文档同步；`config.example.yaml` 默认 `auth-dir: auths`、`panel-github-repository: ""`、`disable-auto-update-panel: true`；`ResolveAuthDir` 对 `auths` 空目录自动回退 `~/.cli-proxy-api`，`~/.starapi-router` 证书目录同样回退，现有登录不丢；`DefaultPanelGitHubRepository=""`、`DefaultDisableAutoUpdatePanel=true`，空仓库不再回落上游 `Cli-Proxy-API-Management-Center`/`cpamc.router-for.me`，UA 改为 `StarAPI-management-updater`；Docker/Compose 镜像改 `starapi-router`、工作目录 `/starapi-router`、示例端口 1991；`FUNDING.yml`/`AGENTS.md`/SDK 文档示例 GitHub 链接同步；插件 ABI `cliproxy_plugin_init`、OAuth client_id、缓存键 `cli-proxy-api:` 与 `LICENSE` 原版权保持不变。
