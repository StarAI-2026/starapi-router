...
- 对话框直接生图：切换生图模型即可在聊天窗口出图，支持图生图与多轮改图
...
- 更聪明的降级链：同一会话锁定首选模型，故障通道自动冷却跳过，触发条件可按需勾选
...
- 可自定义每个模型单价
- 内置题库做模型体检，辅助判断降智或调包
- 按客户端看用量：自动识别请求来自哪个编码工具，勾选后趋势、模型占比与会话明细一起跟着筛
- 可把工具自己记录的本机用量并入同一个总数，直连官方接口的消耗不再是盲区，同一笔不重复计
- 面板内直接查看后端运行日志与本机工具日志，排查问题不用再翻文件
